...
export default class extends Controller {
  connect() {
	...
	$(function () {
            ...
            var data = {
                rss: 'https://medium.com/feed/@renovacionculturalctmae'
            };
            $.get(' https://api.rss2json.com/v1/api.json?rss_url=https%3A%2F%2Fmedium.com%2Ffeed%2F%40renovacionculturalctmae', data, function (response) {
                if (response.status == 'ok') {
                    ...
                    $.each(response.items, function (k, item) {
                        ...
                        display += `<a href="${item.link}" target="_blank" class="btn btn-outline-success" >Leer mas</a>`;
                        display += '</div></div>';
                        return k < 10;
                    });
